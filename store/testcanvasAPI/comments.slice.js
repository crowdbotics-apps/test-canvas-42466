...
export const modules_disqus_comment_create = createAsyncThunk("comments/modules_disqus_comment_create", async payload => {
  const response = await apiService.modules_disqus_comment_create(payload);
  return response.data;
});
export const modules_disqus_comment_retrieve = createAsyncThunk("comments/modules_disqus_comment_retrieve", async payload => {
  const response = await apiService.modules_disqus_comment_retrieve(payload);
  return response.data;
});
export const modules_disqus_comment_update = createAsyncThunk("comments/modules_disqus_comment_update", async payload => {
  const response = await apiService.modules_disqus_comment_update(payload);
  return response.data;
});
export const modules_disqus_comment_partial_update = createAsyncThunk("comments/modules_disqus_comment_partial_update", async payload => {
  const response = await apiService.modules_disqus_comment_partial_update(payload);
  return response.data;
});
export const modules_disqus_comment_destroy = createAsyncThunk("comments/modules_disqus_comment_destroy", async payload => {
...
const commentsSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(modules_disqus_comment_create.pending, (state, action) => {
    ...
    }).addCase(modules_disqus_comment_partial_update.pending, (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending";
      }
    }).addCase(modules_disqus_comment_partial_update.fulfilled, (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = state.entities.map(record => record.id === action.payload.id ? action.payload : record);
        state.api.loading = "idle";
      }
    }).addCase(modules_disqus_comment_partial_update.rejected, (state, action) => {
      if (state.api.loading === "pending") {
        state.api.error = action.error;
        state.api.loading = "idle";
      }
